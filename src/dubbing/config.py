...
def probe_dubbing_runtime(python_path: Path | str | None) -> dict[str, Any]:
    if python_path is None:
        return {"torch_ready": False, "cuda_available": False, "error": ""}
    executable = Path(python_path).resolve()
    ...
    try:
        ...
        result = {"torch_ready": False, "cuda_available": False, "error": str(exc)}
...
def public_dubbing_health(project_root: Path | str) -> dict[str, Any]:
    ...
    runtime_probe = (
        probe_dubbing_runtime(runtime)
        if demucs_ready and voxcpm_ready
        else {"torch_ready": False, "cuda_available": False, "error": ""}
    )
    ...
    return {
        ...
        "runtime_error": str(runtime_probe.get("error") or ""),
        "backend": "voxcpm2",
