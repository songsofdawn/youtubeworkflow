...
class CandidateRanker:
    POTENTIAL_DEFAULTS = {"ai": .42, "user": .16, "relevance": .12, "novelty": .10,
    ...
    def __init__(self, config=None):
        ...
        self.potential_weights = cfg.get("potential_ranking_weights", self.POTENTIAL_DEFAULTS)
        self.hot_weights = cfg.get("hot_ranking_weights", self.HOT_DEFAULTS)
    ...
    def potential_score(self, row):
        ...
        return 100 * sum(float(self.potential_weights.get(k, 0)) * v for k, v in values.items()) - penalty
    ...
    def hot_score(self, row):
        ...
        return 100 * sum(float(self.hot_weights.get(k, 0)) * v for k, v in values.items())
