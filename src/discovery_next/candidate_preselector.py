...
class CandidatePreselector:
    ...
    def select(self, rows, budget):
        ...
        channels, clusters, domains, queries = Counter(), Counter(), Counter(), Counter()
        ...
        def accept(row):
            ...
            if row["video_id"] in chosen_ids or channels[channel] >= self.channel_limit or clusters[cluster] >= self.semantic_limit:
                return False
            chosen.append(row); chosen_ids.add(row["video_id"])
            channels[channel] += 1; clusters[cluster] += 1
            domains[(row.get("matched_pack_ids") or [""])[0]] += 1; queries[primary_query] += 1
