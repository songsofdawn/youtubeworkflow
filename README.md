...
| 中配需要复核时 | 自动改发原声字幕版、阻止后续、仍继续 | 默认自动降级；继续模式需要主动选择并承担语音重叠风险 |
...
队列。选择“阻止后续”仍会按总异常策略跳过或保留失败；只有明确选择“仍继续”才会越过语音
...
直接跳过。该模式不会把无配音素材伪装成双语成片，标题也不再强制添加“【无配音】”。
