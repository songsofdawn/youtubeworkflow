...
- 使用 YouTube Data API 关键词搜索，或用 Ollama 做本地智能发现、结构化内容评价和 Hot / Potential 排序；
- 智能发现提供可编辑领域 taxonomy，支持“自动发现 / 手动聚焦”、“内容潜力 / 热门”和
  “快速 / 标准 / 深度”，从长期画像、近期行为、反馈和查询绩效动态规划领域、意图与预算；
...
- **智能发现**：默认由历史画像自动规划领域、搜索意图和预算，再做多路召回、分桶预选与反馈学习。
...
领域配置为 config/discovery_taxonomy.json，共享形式为 config/discovery_formats.json。taxonomy
是系统内部的组织、覆盖和归因空间，默认自动发现不要求勾选领域；“手动聚焦”仅覆盖本轮策略，
不会写入长期画像。面板仍可在高级入口维护 taxonomy，但用户无需维护搜索句。
...
快速/标准/深度分别映射整轮搜索与 AI 候选预算，并受 discovery_max_search_requests 上限约束。
DomainAllocator 综合长期领域偏好、近期衰减偏好、emerging interest、查询效率、负反馈、覆盖缺口
和画像集中度；探索比例动态保持在配置上下限之间。Qwen 先规划 intent 再生成自然查询，学习概念、
偏好频道、emerging、coverage、exploration 与 cross-domain 共同进入统一召回池；语义近似查询去重，
低绩效查询进入 cooldown，高绩效查询获得更高候选优先级。
...
时间范围仍支持 24 / 72 / 168 / 336 / 720 小时。有限的 Qwen 预算按热门、相关、用户偏好、
新颖、emerging、exploration、低热高相关与稀有领域分桶，不再先按热度截断。热门排序由速度、
播放量、新鲜度、互动率主导并设 AI 质量底线；潜力排序由 AI 内容潜力、用户匹配、相关性、
新颖性和本地化价值主导，热度仅占辅助权重。最终还限制频道和语义簇重复。
Ollama 不可用时搜索回退到公开元数据分类并提示，不会假装完成分析。

使用现有 Ollama qwen3.5:9b，不自动安装模型或调用收费 API。发送内容为公开元数据，
支持上游提供的公开字幕摘要，不读取本地字幕正文。不使用旧 Embedding、视觉复评、固定词池、
查询轮换或 heat-first AI 预选；相关旧界面选项已停用。
...
裁剪首尾静音 → 连续语音区域共享时长预算 → 无重叠时间轴拼接 → 中文人声 two-pass loudnorm
→ 按实际中文时槽精确 dB duck/mix → 最终 two-pass loudnorm → Stage 4 成片
```

默认中文人声目标为 `-18 LUFS`，最终音轨为 `-14 LUFS / -1 dBTP`。配置中的
`background_duck_db=6` 表示只在中文人声时槽内把背景降低约 6 dB，并使用 40 ms attack、
250 ms release 平滑进出；相邻时槽会合并以避免 pumping。Stage 4 只转码/封装这条已处理音轨，
不会再次做响度标准化。以上高级参数位于 `config\dubbing_config.json`。
...
只修改人声 LUFS 会从人声响度阶段继续；只修改 Ducking 会从混音继续；只修改最终 LUFS/True
Peak 只重做最终响度，不会重新运行 Demucs 或 VoxCPM2。
...
投稿使用独立的单上传槽：排队任务按“优先投稿”再按加入时间执行，当前投稿完成后会自动接续
下一条，并继续遵守投稿最短间隔、每日上限和 137022 冷却。在任务卡的 `...` 菜单选择“优先投稿”即可置顶；
右下方“投稿顺序”会显示视频标题和视频 ID，并可用“上移”“下移”“置顶”调整排队顺序。重复
提交同一视频时会复用已有的排队投稿，不会创建重复投稿；已经记录为成功的稿件仍会被阻止重复提交。
...
复核时自动改发原声字幕版；若完整投稿仍无法安全成片，会改用原视频生成投稿信息后继续上传。
...
5. 保持默认的“自动改发原声字幕版”，或按需要选择阻止、带风险继续策略；
...
| 中配需要复核时 | 自动改发原声字幕版；完整投稿失败时改用原视频、中文投稿信息和封面继续投稿 | 默认自动降级；仍继续模式会越过语音重叠警告 |

面板会保存这些选项，并在流程卡中展示本次实际执行链路。加入队列前，后端还会再次检查
VoxCPM2 模型、独立 Python 运行时、PyTorch/CUDA、翻译 API 和投稿环境，不能仅靠修改页面
参数绕过组合限制。

普通的“AI 字幕尚未人工浏览”提示不会阻塞无人值守任务。结构错误、时间轴错误、字幕
裁切或闪读仍不会被伪装成合格字幕成片；当终点是“完整投稿”且异常策略为默认值时，系统会记录
审计原因，改用原视频、原音轨，自动生成中文标题、标签、封面和投稿信息后继续上传。仅字幕/仅成片终点
会保留失败状态，等待重试。自动化投稿
默认公开；需要先检查稿件时，请主动开启“仅自己可见”。

完整投稿默认也会接续处理英文字幕选择失败、无英语语音或仅有标题字幕的视频；普通英文处理
失败不依赖 Whisper 是否生成识别报告即可降级。字幕或成片进程抛出异常时使用同一接续策略。
原视频投稿信息生成仍失败时，任务保留为可重试的失败状态，不会标记为“已跳过”或反复降级。
没有英文字幕、也没有可识别语音时，系统同样保留原视频，自动生成中文标题、标签、封面并投稿，
不会弹出人工审核。

中配默认先使用静音裁剪和连续语音区域调度；若 `dubbing\manifest.json` 仍标记无法安全适配，
无人值守任务会明确记录降级原因，关闭替换音轨，重新生成原声中文字幕成片并继续投稿。该稿件
使用与媒体类型一致的标签和真实说明，不会伪装成中文配音；标题前缀默认留空。若这一级又
因字幕排版或成片错误无法投稿，完整投稿模式会最终保留原始视频、生成本地化投稿信息并继续
队列。完整投稿终点会自动改用原视频生成投稿信息并继续投稿；只有选择“仍继续”才会越过语音
时序警告。正常中配投稿始终读取本次 Stage 4 manifest 记录的中配文件，不按文件名猜测。
...
确认无配音且没有英文字幕的视频，会保留原视频并生成中文标题、标签、封面和“无配音”投稿信息，
不会把无配音素材伪装成双语成片，也不会要求人工审核。
...
标题默认不添加任何前缀，也不再强制要求“【中英双语】”“【原声中字】”或“【无配音】”。
...
在无人值守区或手动任务区选择封面模式后，对应的处理任务会生成中文封面。
现有任务可点击“封”，预览原封面、中文封面、候选文案和日志，并手动重新生成。
无人值守自动化区与视频任务手动区各有独立的“关闭 / 本地 / API 文案 + Pillow”选项，默认关闭。
“一键投稿”及启用自动化后的下载使用无人值守区设置；下载自动化开关不影响“一键投稿”。
普通下载只保存素材和原封面，不生成中文封面，也不要求封面 API 授权；手动字幕/成片/中配操作使用手动区设置。
选项直接记录到新任务，不会被全局封面模式覆盖。区域内选择本地时不调用封面 API；
选择 API 时必须在同一区域授权调用当前翻译供应商；也可关闭封面或改用本地模式后继续处理、投稿。
授权框在 API 模式下可勾选，服务配置未就绪时会显示提示，并在提交时检查 Key；它不会调用图片生成或图片编辑 API。
自动化选择与授权随原自动化策略保存在浏览器；手动模式会记住，手动 API 授权刷新后需重新勾选。
关闭只跳过生成，不删除或停用已有成功封面，不改变已排队任务。预览窗口的单独重新生成仍使用服务配置中的模式与本次授权。
功能默认关闭；封面失败不会阻塞字幕、配音、成片或投稿，投稿会使用原封面。
封面作为独立作业调度：API 文案和随后的快速 Pillow 绘图走 `paid_api` 工作线程，可与同一视频的字幕选择/编码并行；投稿会等该封面作业终止后再领取，因此不会抢先上传旧封面。

本地模式调用链：复用 yt-dlp 的 `metadata/info.json`，按尺寸/优先级下载最高质量可用缩略图
→ 复用已有摘要或生成 `video_context.json` → 本地视觉分析文字和主体、生成候选
→ 可选的云端文案优化 → 安全擦字 → Pillow 绘制中文 → 校验成功记录后供投稿使用。
原封面始终保留；重新生成失败时，旧中文文件也不会作为成功结果继续投稿。

本地分析复用 Ollama 配置和支持图片输入的模型（项目默认 `qwen3.5:9b`），不需要新 API Key。
依赖 Pillow 和中文字体（Windows 默认尝试微软雅黑/黑体）。不会自动下载或启动模型。
遵守项目数据边界：Ollama 仅接收公开元数据与缩略图，不发送字幕；已有字幕分析摘要可供
单独授权的云端文案步骤复用。云端复用当前翻译供应商、模型和 Key，只发送限长摘要、标题、
关键词和封面文字，不重复发送完整字幕。“允许封面付费文案”与字幕翻译授权独立，默认关闭。

本地擦字优先重建简单背景；遇到复杂纹理、文字与主体重叠、大面积或贴边文字时，自动使用
OpenCV Telea 积极修复，不再因这些情况直接放弃。只修改识别的文字区域，原图保留。
这不是生成式修复，不能恢复被遮住的真实面部，可能产生模糊、涂抹或细节失真；风险写入日志和预览。
依赖新增 `opencv-python-headless`。文字定位不可靠、缺少依赖、模型/修复执行失败仍回退原封面。
本地模式最终文字只由 Pillow 排版，自动调整位置、字号、两行内换行、对齐、描边和阴影；中文优选字号为画面高度的 19%，
可发布下限为 9%。识别到的原文字框过窄时会改用更大的顶部、底部或侧边区域，不再把中文缩成字幕大小；
无完全空闲位置时选择较少遮挡的区域并提示风险，不再仅因主体占满画面而放弃。最终文案水平居中、垂直位于画面中下部，
字体颜色从配置色板随机变化，强制重新生成时会避开上一种颜色并把本次颜色写入封面清单。

API 模式调用链：原标题 + 限长摘要 → 当前翻译供应商生成并选出中文候选
→ Pillow 在原封面上绘制大号中文 → 校验成功记录后供投稿使用。该模式不启动 Ollama，
不上传原封面或完整字幕，也不调用 OpenAI Images、智谱文生图或其他图片生成/编辑接口。
中文由本地字体确定性绘制，因此不会出现图片模型常见的中文乱码或漏字。

API 模式直接复用“配置服务 → AI 翻译”中的供应商、模型和 Key。例如选择智谱后使用
`ZHIPU_API_KEY`，不需要 `OPENAI_API_KEY` 或独立封面 Key。默认不授权 API：新任务和手动
重新生成都需在对应区域勾选本次文案 API 授权。失败继续使用原封面，成功图片按哈希复用。

新增配置：`mode=local|cloud`（`cloud` 是为旧任务保留的内部兼容值，页面显示“API 文案 +
Pillow”）、`allow_cloud_api=false`。修改并保存模式后只影响新提交的任务；更新代码后需等
活动任务结束再重启后台，刷新网页不等于重启。
修改并保存模式后只影响新提交的任务；更新代码后需等活动任务结束再重启后台，刷新网页不等于重启。

配置文件 `config/cover_localization_config.json`：`enabled=false`、`allow_paid_copy=false`、
`use_local_vision=true`、`localized_filename="thumbnail.zh.jpg"`、`jpeg_quality=94`、
`max_candidates=5`、`preferred_min_characters=4`、`max_characters=12`、`font_path=""`。
`text_colors` 为封面字体随机色板；留空或全部无效时使用内置高对比色板。
字体路径为空时自动查找中文字体；文案优先 4–8 字，最多 12 字，至少三个有效候选。

单个已有任务可运行（路径替换成实际任务目录）：

```bat
.venv\Scripts\python.exe -m src.run_cover_localization --video-dir "downloads\manual\日期\视频ID_标题"
```

`--force` 忽略缓存；`--allow-paid-api` 明确允许本次调用现有翻译 API。
`--mode cloud --allow-cloud-api` 指定 API 文案 + Pillow 并授权本次文案调用；`--mode local` 指定本地。
离线测试：`.venv\Scripts\python.exe -m unittest tests.test_cover_cloud tests.test_cover_localization tests.test_control_panel tests.test_download_stage2`；前端回归：`node tests/test_cover_panel_ui.cjs`。
产物为 `metadata/thumbnail.jpg`（原图）、`metadata/thumbnail.zh.jpg`（成功的中文图）、
`video_context.json`、`cover/cover_manifest.json`（状态/最终文案/哈希）、
`cover/cover_candidates.json`、`cover/cover_analysis.json`、`cover/cover_localization.log`；
调用 API 文案时另有 `cover/api_usage.json`。缓存按原图、上下文和配置校验。
...
| `dubbing\chinese_voice_normalized.wav` | 整体响度统一后的中文人声 |
| `dubbing\mixed_audio.wav` | Ducking 与混音后的最终响度处理输入 |
...
node tests\test_discovery_queries_ui.cjs
git diff --check
```
